...
# }
#
# data_partitioned <- function(y, f) {
#   cat("reading data.")
#
#   if (y >= 2012) {
#     hs02_count <- open_dataset("../uncomtrade-datasets-arrow/hs-rev2002/parquet",
#                                partitioning = c("aggregate_level", "trade_flow", "year", "reporter_iso")) %>%
#       filter(
#         year == y,
#         aggregate_level == 6
#       ) %>%
#       select(reporter_iso, partner_iso, reporter_code, partner_code) %>%
#       distinct() %>%
#       collect() %>%
#       fix_missing_iso()
#
#     hs12_count <- open_dataset("../uncomtrade-datasets-arrow/hs-rev2012/parquet",
#                                partitioning = c("aggregate_level", "trade_flow", "year", "reporter_iso")) %>%
#       filter(
#         year == y,
#         aggregate_level == 6
...
#       open_dataset("../uncomtrade-datasets-arrow/hs-rev2012/parquet",
#                    partitioning = c("aggregate_level", "trade_flow", "year", "reporter_iso"))
#     } else {
#       open_dataset("../uncomtrade-datasets-arrow/hs-rev2002/parquet",
#                    partitioning = c("aggregate_level", "trade_flow", "year", "reporter_iso"))
#     }
#
...
#       aggregate_level == 6
#     ) %>%
#     select(reporter_iso, partner_iso, reporter_code, partner_code, commodity_code, trade_value_usd) %>%
#     collect() %>%
#     # need to pass gsub one by one bc arrow translation won't accept the
#     # clear_hive function above
#     filter(reporter_iso != "wld", partner_iso != "wld") %>%
#     fix_missing_iso()
#   cat(".\n")
#
...
#
#     d <- d %>%
#       left_join(product_correlation, by = c("commodity_code" = "hs02"))
#     cat(".")
#
#     d <- d %>%
#       select(reporter_iso, partner_iso, commodity_code = hs12, trade_value_usd) %>%
#       mutate(
...
#     rename(trade_value_usd_imp = trade_value_usd.x, trade_value_usd_exp = trade_value_usd.y) %>%
#
#     mutate(
...
#       trade_value_usd_total = trade_value_usd_imp + trade_value_usd_exp
#     ) %>%
#
#     filter(trade_value_usd_total > 0) %>%
#     select(-trade_value_usd_total)
#
#   return(d)
# }
#
...
#   d %>%
#     filter(
#       trade_value_usd_imp >= val & trade_value_usd_exp >= val
#     )
# }
#
# conciliate_flows <- function(y) {
#   cat("IMPORTS\n")
#   dimp <- data_partitioned(y = y, f = "import")
...
#
#   cat("EXPORTS\n")
#   dexp <- data_partitioned(y, f = "export")
...
#           trade_value_usd_imp != 0 ~ "both parties",
#         trade_value_usd_exp != 0 &
#           trade_value_usd_imp == 0 ~ "exporter only",
#         trade_value_usd_exp == 0 &
#           trade_value_usd_imp != 0 ~ "importer only"
#       )
#     )
# }
#
